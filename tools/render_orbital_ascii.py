...
# that core. This table names the nearest filled shell below common closures.
CORE_SHELLS = {
    2: ["0s1/2"],
    8: ["0p3/2", "0p1/2"],
    20: ["0d5/2", "1s1/2", "0d3/2"],
    28: ["0f7/2"],
    50: ["0f5/2", "1p3/2", "1p1/2", "0g9/2"],
    82: ["0g7/2", "1d5/2", "1d3/2", "2s1/2", "0h11/2"],
    126: ["0h9/2", "1f7/2", "1f5/2", "2p3/2", "2p1/2", "0i13/2"],
}
...
def read_snt(path: Path) -> tuple[dict[str, int], dict[str, list[dict[str, object]]]]:
    ...
    for species in orbitals.values():
        species.sort(key=lambda item: (item["spe"] is None, item["spe"] or 0.0), reverse=True)
...
def format_model_space_row(entry: dict[str, object]) -> str:
    ...
    return f"{entry['label']:<7} [{int(entry['capacity']):>2}] {spe_text:>12}"


def format_core_row(label: str) -> str:
    capacity = capacity_from_label(label)
    if capacity is None:
        return f"{label:<7}       inert core"
    return f"{label:<7} [{capacity:>2}]  inert core"
...
def render_ascii(name: str, snt_path: Path) -> str:
    ...
    proton_core = [format_core_row(label) for label in reversed(CORE_SHELLS.get(core["z"], []))]
    neutron_core = [format_core_row(label) for label in reversed(CORE_SHELLS.get(core["n"], []))]
