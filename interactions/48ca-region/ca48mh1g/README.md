...
`ca48mh1g` is a KSHELL-format interaction for calculations around a `48Ca` core. The file uses a `Z=20, N=28` core, with proton particles in the full `pf` shell and neutron particles in the upper-`pf` orbitals plus `0g9/2`.

This entry is a local `ca48mh1`-family variant with a shifted neutron `0g9/2` SPE.

## Orbital Space

```text
CA48MH1G orbital space (KSHELL file convention)
Core: Z=20, N=28
  ...
  pf   1p1/2   [ 2]   -5.133 MeV           sdg  0g9/2   [10]    1.700 MeV
...
The model space matches the `48Ca`-core `fpg9/2` space commonly described in the literature around the Sorlin et al. `fpg` interaction. The exact origin of the local `ca48mh1g` file label is not established from the KSHELL header or from a public exact-name search.

Midtboe's dissertation explicitly identifies `CA48MH1g` as the `CA48MH1` interaction with the neutron `0g9/2` single-particle energy shifted upward for the `70Ni` calculations.

## Notes

`ca48mh1g` uses the same active orbitals as `ca48mh1`, and the same proton SPEs. Relative to `ca48mh1`, the neutron `0g9/2` SPE is moved from `-1.795 MeV` to `1.700 MeV` in the copied file.

The dissertation reports that this modified interaction reproduces important low-lying `70Ni` features, including the excited `0+` state near `1.5 MeV` and the onset of negative-parity states around `3 MeV`.

Local source checksum:

```text
59d4908898f06e653e755bf98d64e29902b649b0  ca48mh1g.snt
```

## Relevant Literature

- O. Sorlin et al., "`^{68}_{28}Ni_{40}: Magicity versus Superfluidity`," Phys. Rev. Lett. 88, 092501 (2002). Standard literature trail for the `48Ca`-core `fpg` interaction family; not named in the local `ca48mh1g.snt` header. DOI: [10.1103/PhysRevLett.88.092501](https://doi.org/10.1103/PhysRevLett.88.092501)
...
- A. C. Larsen, J. E. Midtboe, M. Guttormsen et al., "Enhanced low-energy gamma-decay strength of 70Ni and its robustness within the shell model," Phys. Rev. C 97, 054329 (2018). Uses and discusses `CA48MH1G` in detail. DOI: [10.1103/PhysRevC.97.054329](https://doi.org/10.1103/PhysRevC.97.054329)
...
- [files/kshell/ca48mh1g.snt](files/kshell/ca48mh1g.snt)

## Citation

For calculations using this exact file, record the local file name and checksum. Until the exact NuShellX `ca48mh1g` label provenance is resolved, cite the `48Ca`-core `fpg` interaction literature beginning with Sorlin et al., Phys. Rev. Lett. 88, 092501 (2002), DOI: [10.1103/PhysRevLett.88.092501](https://doi.org/10.1103/PhysRevLett.88.092501).
