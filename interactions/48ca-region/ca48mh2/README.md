...
`ca48mh2` is a KSHELL-format interaction for calculations around a `48Ca` core. The file uses a `Z=20, N=28` core, with proton particles in the full `pf` shell and neutron particles in the upper-`pf` orbitals plus `0g9/2`.

This entry is the base `ca48mh2` member in the local `ca48mh*` group. Compared with the `ca48mh1` family, it changes the neutron SPE set and several two-body matrix elements.

## Orbital Space

```text
CA48MH2 orbital space (KSHELL file convention)
Core: Z=20, N=28
  ...
  pf   1p1/2   [ 2]   -5.133 MeV           sdg  0g9/2   [10]   -0.173 MeV
...
```text
2485c3b3f08ae16e2adfa872afbd5c6d155629bd  ca48mh2.snt
```

## Relevant Literature

- O. Sorlin et al., "`^{68}_{28}Ni_{40}: Magicity versus Superfluidity`," Phys. Rev. Lett. 88, 092501 (2002). Standard literature trail for the `48Ca`-core `fpg` interaction family; not named in the local `ca48mh2.snt` header. DOI: [10.1103/PhysRevLett.88.092501](https://doi.org/10.1103/PhysRevLett.88.092501)
...
- [files/kshell/ca48mh2.snt](files/kshell/ca48mh2.snt)

## Citation

For calculations using this exact file, record the local file name and checksum. Until the exact NuShellX `ca48mh2` label provenance is resolved, cite the `48Ca`-core `fpg` interaction literature beginning with Sorlin et al., Phys. Rev. Lett. 88, 092501 (2002), DOI: [10.1103/PhysRevLett.88.092501](https://doi.org/10.1103/PhysRevLett.88.092501).
